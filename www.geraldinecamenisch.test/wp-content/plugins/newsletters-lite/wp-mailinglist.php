...
if (!class_exists('wpMail')) {
	class wpMail extends wpMailPlugin {
		...
		function tiny_mce_before_init($init_array = array()) {
			...
			$init_array['newsletters_posts_orderby_values'] = json_encode($newsletters_posts_orderby_values);
		...
		function countries_hook() {
			...
			echo sprintf(__('%s countries saved to subscribers from their IP address.', 'wp-mailinglist'), $countriessaved);			
			return true;
		}
		...
		function admin_notices() {
			...
			if (is_admin()) {
				...
				if (!empty($_GET['newsletters_exportlink'])) {
					$message = sprintf(__('Your export is ready. %s', 'wp-mailinglist'), '<a class="button button-secondary" href="' . $Html -> retainquery('wpmlmethod=exportdownload&file=' . esc_html($_GET['newsletters_exportlink']), $this -> url) . '"><i class="fa fa-download fa-fw"></i>' . __('Download', 'wp-mailinglist') . '</a>');
		...
		function init() {
			...
			$wpmlmethod = (empty($_POST[$this -> pre . 'method'])) ? null : esc_html($_POST[$this -> pre . 'method']);
			...
			if (!empty($_GET[$this -> pre . 'link']) || !empty($_GET['newsletters_link'])) {
				$link_hash = (empty($_GET[$this -> pre . 'link'])) ? esc_html($_GET['newsletters_link']) : esc_html($_GET[$this -> pre . 'link']);
				
				if ($link = $this -> Link() -> find(array('hash' => $link_hash))) {

					$email_conditions = array('history_id' => esc_html($_GET['history_id']));
					if (!empty($_GET['subscriber_id'])) { $email_conditions['subscriber_id'] = esc_html($_GET['subscriber_id']); }
					if (!empty($_GET['user_id'])) { $email_conditions['user_id'] = esc_html($_GET['user_id']); }
				...
				switch ($_GET['newsletters_method']) {
					case 'newsletter'					:
						...
						if (!empty($id)) {
							if ($email = $this -> History() -> find(array('id' => $id))) {
								...
								$content = $this -> render_email('send', array('print' => esc_html($_GET['print']), 'message' => $message, 'subject' => $email -> subject, 'subscriber' => $subscriber, 'history_id' => $id), false, true, true, $email -> theme_id);
								$output = "";
								ob_start();
					...
					case 'set_user_option'				:
						if (!empty($_GET['option']) && !empty($_GET['value'])) {
							$this -> update_user_option(get_current_user_id(), esc_html($_GET['option']), esc_html($_GET['value']));
						}

						$this -> redirect($Html -> retainquery(esc_html($_GET['option']) . '=' . urlencode(esc_html($_GET['value'])), $this -> referer));
						break;
					...
					case 'management_login'				:
						...
						if (!empty($_POST)) {
							if (!empty($_POST['email'])) {
								if ($Subscriber -> email_validate(esc_html($_POST['email']))) {
									$Db -> model = $Subscriber -> model;

									if ($subscriber = $Db -> find(array('email' => $_POST['email']))) {
										if ($subscriberauth = $this -> gen_auth($subscriber -> id)) {
											$Auth -> set_emailcookie(esc_html($_POST['email']));
					...
					case 'delete_transient'				:
						if (!empty($_GET['transient'])) {
							delete_transient(esc_html($_GET['transient']));
					...
					case 'hidemessage'					:
						if (!empty($_GET['message'])) {
							...
							switch ($_GET['message']) {
								...
								case 'dbupdate'					:
									$this -> delete_option('showmessage_dbupdate');
									$this -> update_option('hidedbupdate', esc_html($_GET['version']));
									break;
								case 'queue_status'				:
								...
								if (!empty($_POST)) {
									if (!empty($_POST['event'])) {
										...
										if (!empty($_POST['recipient'])) {
											$subscriber = $Subscriber -> get_by_email(esc_html($_POST['recipient']));
										}
										...
										switch ($_POST['event']) {
											...
											case 'clicked'				:
												...
												break;
											case 'complained'			:
												$this -> bounce(esc_html($_POST['recipient']), "mailgun", __('Spam complaint', 'wp-mailinglist'), $custom_data -> MessageID);
												break;
											case 'bounced'				:
												$this -> bounce(esc_html($_POST['recipient']), "mailgun", esc_html($_POST['code']) . ' - ' . esc_html($_POST['error']), $custom_data -> MessageID);
												break;
											case 'dropped'				:
												$this -> bounce(esc_html($_POST['recipient']), "mailgun", esc_html($_POST['code']) . ' - ' . esc_html($_POST['description']), $custom_data -> MessageID);
												break;
											case 'unsubscribed'			:
												// subscriber has unsubscribed
												if (!empty($subscriber)) {
			...
			if (!empty($method)) {
				switch ($method) {
					...
					case 'themebyname'		:
						if (!empty($_GET['name'])) {
							ob_start();
							include $this -> plugin_base() . DS . 'includes' . DS . 'themes' . DS . esc_html($_GET['name']) . DS . 'index.html';
					...
					case 'offsite'			:
						...
						if (!empty($form_id) && $form = $this -> Subscribeform() -> find(array('id' => $form_id))) {
							$form -> ajax = false;
							$form -> offsite = true;
							$this -> render('offsite', array('form' => $form, 'title' => __($form -> title)), true, 'default');
							
						} elseif (!empty($list)) {
							$atts['list'] = esc_html($_GET['list']);
								...
								if (empty($_POST['form_id'])) {									
									if ($this -> get_option('subscriberedirect') == "Y") {
										...
										if (!empty($_POST['list_id']) && (!is_array($_POST['list_id']) || count($_POST['list_id']) == 1)) {
											if ($subscribelist = $Mailinglist -> get(esc_html($_POST['list_id'][0]))) {
												if (!empty($subscribelist -> subredirect)) {
													$subscriberedirecturl = __($subscribelist -> subredirect);
												}
											}
									...
									if ($form = $this -> Subscribeform() -> find(array('id' => esc_html($_POST['form_id'])))) {
										if (empty($form -> confirmationtype) || $form -> confirmationtype == "message") {
											if (!empty($form -> confirmation_message)) {
					...
					case 'unsubscribe'		:
						...
						$querystring = 'method=unsubscribe&' . $this -> pre . 'subscriber_id=' . esc_html($_GET[$this -> pre . 'subscriber_id']) . '&' . $this -> pre . 'mailinglist_id=' . esc_html($_GET[$this -> pre . 'mailinglist_id']) . '&authkey=' . esc_html($_GET['authkey']);
					...
					case 'activate'			:					
						...
						if (!empty($_GET[$this -> pre . 'subscriber_id']) && !empty($_GET['authkey'])) {
							...
							$authkey = sanitize_text_field($_GET['authkey']);

							if (!empty($_GET[$this -> pre . 'mailinglist_id'])) {
								$mailinglists = @explode(",", esc_html($_GET[$this -> pre . 'mailinglist_id']));
								$mailinglistsstring = esc_html($_GET[$this -> pre . 'mailinglist_id']);
							}
						...
						if (!empty($mailinglists) && count($mailinglists) == 1 && !empty($mailinglist -> redirect)) {
							$activateredirecturl = __($mailinglist -> redirect);
						} else {
					...
					case 'paypal'			:
						...
						// Let's try PHP CURL to see if it works
						if (function_exists('curl_init')) {
							...
							if ( !($res = curl_exec($ch)) ) {
								$this -> log_error("Got " . curl_error($ch) . " when processing IPN data");
								curl_close($ch);
							}
							
							curl_close($ch);
						// If PHP CURL is not available, let's use a socket connection
						} else {
							$ppport = 443;
							$header = '';
					...
					case 'twocheckout'		:
						if (!empty($_POST['order_number'])) {
							if ($_POST['credit_card_processed'] == "Y") {
								...
								$total = esc_html($_POST['total']);

								if ($_POST['demo'] == "Y" && $this -> get_option('tcodemo') == "Y") {
									$ordernumber = 1;
								} else {
									$ordernumber = esc_html($_POST['order_number']);
								}
								...
								if ($mykey === $_POST['key']) {
									$subscriberid = esc_html($_POST['subscriber_id']);
									$subscriber = $Subscriber -> get($subscriberid, false);
									$mailinglist_id = esc_html($_POST['mailinglist_id']);
									...
									$orderdata = array(
										...
										'completed'				=>	'Y',
										'amount'				=>	esc_html($_POST['total']),
										'product_id'			=>	1,
										'order_number'			=>	esc_html($_POST['order_number']),
						...
						switch ($_GET['type']) {
							case 'mandrill'			:
								if (isset($_POST['mandrill_events'])) {
									$events = json_decode(stripslashes(esc_html($_POST['mandrill_events'])));
		...
		function autoresponders_hook() {
			...
			if (!empty($autoresponderemails)) {
				foreach ($autoresponderemails as $ae) {
					...
					if (!empty($ae -> list_id)) {
						if ($sl = $Db -> find(array('list_id' => $ae -> list_id, 'subscriber_id' => $ae -> subscriber_id))) {
							if (!empty($sl -> active) && $sl -> active == "N") {
								//don't do this autoresponder, the subscription is active
								continue;
							}
						}
					}
		...
		function plugin_action_links($actions = null, $plugin_file = null, $plugin_data = null, $context = null) {
			...
			if (!empty($plugin_file) && $plugin_file == $this_plugin) {
				$actions[] = '<a href="" onclick="jQuery.colorbox({href:newsletters_ajaxurl + \'action=' . $this -> pre . 'serialkey\'}); return false;" id="' . $this -> pre . 'submitseriallink" title="' . __('Serial Key', 'wp-mailinglist') . '"><i class="fa fa-key fa-fw"></i>' . __('Serial Key', 'wp-mailinglist') . '</a>';
		...
		function admin_menu() {
			...
			$this -> menus['newsletters-gdpr'] = add_submenu_page($this -> menus['newsletters-settings'], __($menunames['newsletters-gdpr']), __($menunames['newsletters-gdpr']), 'newsletters_gdpr', $this -> sections -> gdpr, array($this, 'admin_gdpr'));
		...
		function admin_head_welcome() {
			...
			do_action('do_meta_boxes', "newsletters_page" . $this -> sections -> welcome, 'side', $post);
			do_action('do_meta_boxes', "newsletters_page" . $this -> sections -> welcome, 'normal', $post);
			do_action('do_meta_boxes', "newsletters_page" . $this -> sections -> welcome, 'advanced', $post);
		}
		...
		function admin_head_forms() {
			...
			do_action('do_meta_boxes', "newsletters_page" . $this -> sections -> forms, 'side', $post);
			do_action('do_meta_boxes', "newsletters_page" . $this -> sections -> forms, 'normal', $post);
			do_action('do_meta_boxes', "newsletters_page" . $this -> sections -> forms, 'advanced', $post);
		}
		...
		function admin_head_send() {
			...
			if (!empty($method) && $method == "history" && !empty($history_id)) {
				if ($contentareas = $this -> Content() -> find_all(array('history_id' => $history_id), false, array('number', "ASC"))) {
					foreach ($contentareas as $contentarea) {
						add_meta_box('contentareabox' . $contentarea -> number, '<i class="fa fa-file fa-fw"></i> ' . sprintf(__('Content Area %s', 'wp-mailinglist'), $contentarea -> number), array($Metabox, 'send_contentarea'), "newsletters_page_" . $this -> sections -> send, 'normal', 'high', array('contentarea' => $contentarea));
					}
				}
			}
			...
			do_action('do_meta_boxes', "newsletters_page_" . $this -> sections -> send, 'side', $post);
			do_action('do_meta_boxes', "newsletters_page_" . $this -> sections -> send, 'normal', $post);
			do_action('do_meta_boxes', "newsletters_page_" . $this -> sections -> send, 'advanced', $post);
		}
		...
		function admin_head_templates_save() {
			...
			do_action('do_meta_boxes', "admin_page_" . $this -> sections -> templates_save, 'side', $post);
			do_action('do_meta_boxes', "admin_page_" . $this -> sections -> templates_save, 'normal', $post);
			do_action('do_meta_boxes', "admin_page_" . $this -> sections -> templates_save, 'advanced', $post);
		}
		...
		function admin_head_themes_save() {
			...
			do_action('do_meta_boxes', "admin_page_" . $this -> sections -> themes, 'side', $post);
			do_action('do_meta_boxes', "admin_page_" . $this -> sections -> themes, 'normal', $post);
			do_action('do_meta_boxes', "admin_page_" . $this -> sections -> themes, 'advanced', $post);
		}
		...
		function admin_head_settings() {
			...
			do_action('do_meta_boxes', "newsletters_page_" . $this -> sections -> settings, 'side', $post);
			do_action('do_meta_boxes', "newsletters_page_" . $this -> sections -> settings, 'normal', $post);
			do_action('do_meta_boxes', "newsletters_page_" . $this -> sections -> settings, 'advanced', $post);
		}
		...
		function admin_head_settings_templates() {
			...
			do_action('do_meta_boxes', "newsletters_page_" . $this -> sections -> settings_templates, 'side', $post);
			do_action('do_meta_boxes', "newsletters_page_" . $this -> sections -> settings_templates, 'normal', $post);
			do_action('do_meta_boxes', "newsletters_page_" . $this -> sections -> settings_templates, 'advanced', $post);
		}
		...
		function admin_head_settings_subscribers() {
			...
			add_meta_box('importdiv', __('Import Settings', 'wp-mailinglist'), array($Metabox, 'settings_import'), "newsletters_page_" . $this -> sections -> settings_subscribers, 'normal', 'core');
			...
			do_action('do_meta_boxes', "newsletters_page_" . $this -> sections -> settings_subscribers, 'side', $post);
			do_action('do_meta_boxes', "newsletters_page_" . $this -> sections -> settings_subscribers, 'normal', $post);
			do_action('do_meta_boxes', "newsletters_page_" . $this -> sections -> settings_subscribers, 'advanced', $post);
		}
		...
		function admin_head_settings_extensions_settings() {
			...
			do_action('do_meta_boxes', "newsletters_page_" . $this -> sections -> extensions_settings, 'side', $post);
			do_action('do_meta_boxes', "newsletters_page_" . $this -> sections -> extensions_settings, 'normal', $post);
			do_action('do_meta_boxes', "newsletters_page_" . $this -> sections -> extensions_settings, 'advanced', $post);
		}
		...
		function admin_submitserial() {
			...
			if (!empty($_POST)) {
				...
				if (empty($_REQUEST['serial'])) { $errors[] = __('Please fill in a serial key.', 'wp-mailinglist'); }
					...
					$this -> update_option('serialkey', esc_html($_REQUEST['serial']));	//update the DB option
		...
		function admin_send() {
			...
			if (!empty($_POST['publishpost']) && $_POST['publishpost'] == "Y") {
				$status = (!empty($_POST['post_status'])) ? esc_html($_POST['post_status']) : 'draft';
				$slug = (!empty($_POST['post_slug'])) ? esc_html($_POST['post_slug']) : $Html -> sanitize($_POST['subject'], '-');
				...
				default				:
					if (!empty($_POST['newsletter'])) {
						...
						if (empty($errors)) {
							...
							$defaulttexton = esc_html($_POST['defaulttexton']);
							if (!empty($defaulttexton) && !empty($_POST['customtext'])) {
								$this -> update_option('defaulttexton', true);
								$this -> update_option('defaulttextversion', $_POST['customtext']);
							} else {
								...
								if (!empty($_POST)) {
									if (!empty($errors)) {
										...
										if (empty($_POST['sendtype']) || $_POST['sendtype'] == "queue" || $_POST['sendtype'] == "send") {
											...
											$message = sprintf(__('Newsletter has been scheduled for %s', 'wp-mailinglist'), esc_html($_POST['senddate']));
									...
									foreach ($emails as $email) {
										...
										if (is_email($email)) {
											...
											if (!$this -> execute_mail($subscriber, false, $subject, $message, $newattachments, $history_id, $eunique, false, "preview")) {
												...
												$this -> redirect(admin_url('admin.php?page=' . $this -> sections -> send . '&method=history&id=' . $history_id . '&previewemail=' . esc_html($_POST['previewemail'])), 'message', 1, false);
											}
										} else {
		...
		function admin_mailinglists() {
			...
			switch ($method) {
				...
				case 'offsite'			:
					...
					$this -> redirect($this -> url . '&method=offsitewizard&listid=' . esc_html($_GET['listid']), $msg_type, $message);
					break;
				case 'offsitewizard'	:
			...
			switch ($method) {
				...
				case 'mass'						:
					...
					if (!empty($_POST['groupslist'])) {
						if (!empty($_POST['action'])) {
							$groups = esc_html($_POST['groupslist']);
			...
			switch ($method) {
				case 'save'			:
					if (!empty($_POST)) {
						...
						$subscriber_id = $_POST['Subscriber']['id'];
						if (!empty($subscriber_id)) {
							$subscriber = $Subscriber -> get($subscriber_id);
				...
				default			:
					...
					if (!empty($_POST['screenoptions'])) {
						...
						if (!empty($_POST['perpage'])) {
							$oldperpage = esc_html($_POST['perpage']);
						}
					}
					...
					if (!empty($newsletters_filter_subscribers)) {
						...
						//** country filter
						$newsletters_filter_subscribers_country = (empty($_GET['country'])) ? $_COOKIE['newsletters_filter_subscribers_country'] : $_GET['country'];
					...
					case 'import'			:
						...
						if (empty($error)) {
							...
							if ($fh = fopen($fileFull, "r")) {
								...
								$delimiter = (empty($_POST['delimiter'])) ? $csvdelimiter : esc_html($_POST['delimiter']);
								$d = 0;
								$i_queries = array();
								...
								$uploadedfile = esc_html($_POST['uploadedfile']);
					...
					case 'export'				:
						...
						if (empty($errors)) {
							...
							if (!empty($subscribers)) {
								...
								$csvdelimiter = $this -> get_option('csvdelimiter');
								$delimiter = (!empty($_POST['export_delimiter'])) ? esc_html($_POST['export_delimiter']) : $csvdelimiter;
				...
				case 'exportsent'		:
					...
					if (!empty($_GET['history_id'])) {
						...
						if ($emails = $Db -> find_all(array('history_id' => $_GET['history_id']), false, array('modified', "DESC"))) {
							...
							if (!empty($data)) {
								$exportfile = 'history' . esc_html($_GET['history_id']) . '-emails-' . date_i18n("Ymd") . '.csv';
								$exportpath = $Html -> uploads_path() . DS . $this -> plugin_name . DS . 'export' . DS;
								$exportfull = $exportpath . $exportfile;
			...
			switch ($method) {
				...
				default					:
					...
					if (!empty($_GET['showall'])) {
						...
						$searchterm = (empty($_POST['searchterm'])) ? $searchterm : esc_html($_POST['searchterm']);
