	...
	class wpMailPlugin extends wpMailCheckinit {
		...
		function __construct() {			
			...
			if (class_exists('WP_Queue_Process')) {
				$this -> queue_process = new WP_Queue_Process();
			}
			    ...
			    if (function_exists('file_get_contents')) {
				    ...
				    switch ($random) {
					    case 1					:
					 		...
					 		if ($ipinfo = file_get_contents($url)) {
					    ...
					    case 3					:
					    	...
					    	if ($ipinfo = file_get_contents($url)) {
						    	if (!empty($ipinfo)) {
							    	...
							    	$ipcountry = $ipinfo['geoplugin_countryCode'];
							    }
					    	}
					    	break;
				    }
			...
			if (apply_filters('newsletters_updates', true)) {				
				if ($update = $this -> vendor('update')) {						
			        ...
			        if(empty($version_info['is_valid_key']) || version_compare($this -> version, $version_info["version"], '>=')){
			            ...
			            //$option -> response[$plugin_path] -> package = 'https://downloads.wordpress.org/plugin/newsletters-lite.zip';
			...
			if ($movefile && !isset($movefile['error'])) {
			    ...
			    if (!empty($filetype['type']) && in_array($filetype['type'], $csvtypes)) {
					...
					if (function_exists('mb_detect_encoding')) {
						$encoding = mb_detect_encoding($preview, mb_detect_order(), true);
	    ...
	    function ajax_get_country() {
		    ...
		    if (!empty($_POST['subscriber_id'])) {
			    $subscriber_id = esc_html($_POST['subscriber_id']);
		    ...
		    if (!empty($_POST)) {
			    ...
			    if ($mailinglists = $Mailinglist -> select(true)) {
				    foreach ($mailinglists as $id => $title) {
						...
						$checklist .= '<label><input ' . ($id == $insertid || (!empty($_POST['Subscriber']['mailinglists']) && in_array($id, $_POST['Subscriber']['mailinglists'])) ? 'checked="checked"' : '') . ' type="checkbox" name="' . $fieldname . '[]" value="' . $id . '" id="checklist' . $id . '" /> ' . __($title) . ' (' . $Db -> count(array('list_id' => $id)) . ' ' . __('subscribers', 'wp-mailinglist') . ')</label><br/>';
				    }
			    }
			    ...
			    if ($posts = get_posts($arguments)) {
				    ...
				    <ul class="insertfieldslist">
				    	<li>
				    		...
				    		<span class="">
				    			<label for="checkall_insertposts" style="font-weight:bold;"><?php _e('Select All', 'wp-mailinglist'); ?></label>
				    		</span>
				    	</li>
				    ...
				    <span><input onclick="insert_single_multiple();" class="button button-primary" type="button" name="insert" value="<?php _e('Insert Selected', 'wp-mailinglist'); ?>" /></span>
	    ...
	    function ajax_welcomestats() {
		    ...
		    $chart = (empty($_GET['chart'])) ? "bar" : esc_html($_GET['chart']);
		    $type = (empty($_GET['type'])) ? "days" : esc_html($_GET['type']);
			$fromdate = (empty($_GET['from'])) ? date_i18n("Y-m-d", strtotime($Html -> gen_date("Y-m-d H:i:s", false, false, true) . " -13 days")) : esc_html($_GET['from']);
			$todate = (empty($_GET['to'])) ? date_i18n("Y-m-d") : esc_html($_GET['to']);
			$history_id = (empty($_GET['history_id'])) ? false : esc_html($_GET['history_id']);
						...
						if (!empty($_POST['list_id']) && (!is_array($_POST['list_id']) || count($_POST['list_id']) == 1)) {
							if ($subscribelist = $Mailinglist -> get($_POST['list_id'][0])) {
								if (!empty($subscribelist -> subredirect)) {
									$subscriberedirecturl = __($subscribelist -> subredirect);
								}
							}
						}
					...
					if ($form = $this -> Subscribeform() -> find(array('id' => $_POST['form_id']))) {
						...
						if (empty($form -> confirmationtype) || $form -> confirmationtype == "message") {
							...
							if (!empty($form -> confirmation_redirect)) {
								$redirect = do_shortcode(stripslashes(__($form -> confirmation_redirect)));
		...
		function getbodyandcss($html = null) {
			...
			$d -> loadHTML('<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />' . stripslashes($html));
				...
				if ($form = $this -> Subscribeform() -> find(array('id' => $id))) {
					...
					<html>
						<head>
							...
							<title><?php echo urldecode(esc_html($_GET['title'])); ?></title>
		...
		function ajax_testsettings() {
			...
			if (empty($_GET['init']) && !empty($_POST)) {
				...
				elseif (!$Subscriber -> email_validate($_POST['testemail'])) { $errors[] = __('Please fill in a valid email address', 'wp-mailinglist'); }
					...
					if (!empty($_POST['testattachment']) && $_POST['testattachment'] == 1) {
						$attachments = array(
							array(
								...
								'filename'				=>	$this -> plugin_base() . DS . 'images' . DS . 'testattachment.jpg',
							)
						);
					}
		...
		function ajax_mailapi_amazonses_action() {
			...
			if (!empty($_REQUEST['ac'])) {				
				require($this -> plugin_base() . DS . 'vendor' . DS . 'autoload.php');
				$ses = new SimpleEmailService($_REQUEST['key'], $_REQUEST['secret'], 'email.' . $_REQUEST['region'] . '.amazonaws.com', false);
		...
		function ajax_testbouncesettings() {
			...
			$type = (empty($_POST['type'])) ? 'imap' : $_POST['type'];
			$host = $_POST['host'];
			$user = $_POST['user'];
			$pass = $_POST['pass'];
			$port = $_POST['port'];
	        ...
	        if ($pop3 -> connect($_POST['host'], $_POST['port'])) {
	        	if ($pop3 -> user($_POST['user'])) {
	        		...
	        		if ($count === false) {
		        		...
		        		$success = true;
		        		$message = __('There are no emails in the mailbox', 'wp-mailinglist');
	        		} elseif ($count > 0) {
		        		$success = true;
		        		$message = sprintf(__('There are %s emails in the mailbox', 'wp-mailinglist'), $count);
	        		}
	        	} else {
		        	$success = false;
		        	$error = $pop3 -> ERROR;
		...
		function ajax_addcontentarea() {
			...
			<div class="postbox">
				<h3 class="hndle"><span><?php echo sprintf(__('Content Area %s', 'wp-mailinglist'), esc_html($_POST['contentarea'])); ?></span></h3>
				<div class="inside">
					<?php

					$settings = array(
						'media_buttons'		=>	true,
						'textarea_name'		=>	'contentarea[' . esc_attr($_POST['contentarea']) . ']',
						'textarea_rows'		=>	10,
						'quicktags'			=>	true,
					);

					wp_editor(false, 'contentarea' . esc_attr($_POST['contentarea']), $settings);
		...
		function ajax_newsletter_autosave() {
			define('DOING_AJAX', true);
			define('SHORTINIT', true);
			...
			$history_data = array(
				...
				'selectedcountries'	=>	maybe_serialize($_POST['selectedcountries']),
	    	...
	    	$textpreview = $this -> ajax_historyiframe(true, true);
			...
			<p id="spamscore_report_link_holder" style="text-align:center; display:block;"><a href="#spamscore_report" onclick="jQuery.colorbox({inline:false, html:jQuery('#spamscore_report').html(), maxWidth:'80%', maxHeight:'80%'}); return false;"><?php _e('See Report', 'wp-mailinglist'); ?></a></p>
			...
			<div style="display:none;">
				<div id="spamscore_report">
					<div class="wrap newsletters">
						<h2><?php _e('Spam Score Report', 'wp-mailinglist'); ?></h2>
						<p><a href="" onclick="jQuery.colorbox.close(); return false;" class="button button-primary"><?php _e('Close Report', 'wp-mailinglist'); ?></a></p>
						...
						<h3><?php _e('RAW Email', 'wp-mailinglist'); ?></h3>
						<p><a href="" onclick="jQuery('#rawemail-holder').toggle(); return false;" class="button button-secondary"><?php _e('Toggle RAW Email', 'wp-mailinglist'); ?></a></p>
		...
		function ajax_previewrunner($justsave = false) {
	    	...
	    	<result>
				...
				<textcontent><![CDATA[<?php echo $preview; ?>]]></textcontent>
				<newsletter_url><![CDATA[<?php echo $Html -> retainquery('newsletters_method=newsletter&id=' . $history_id, home_url()); ?>]]></newsletter_url>
			</result>
	    ...
	    function ajax_history_download() {
			...
			if (!empty($id)) {			
				...
				header("Content-Disposition: attachment; filename=\"" . __($email -> subject) . ".html\";");
		...
		function ajax_historyiframe($returnoutput = false, $text = false) {
			define('DOING_AJAX', true);
			define('SHORTINIT', true);
			global $Db, $Subscriber;
			...
			if (!empty($text)) {				
				...
				if (!empty($email -> text)) {					
					...
					$altbody = $this -> process_set_variables($subscriber, $user, stripslashes($altbody), $id, $eunique);
				...
				if ($subscriber = $Db -> find(array('id' => $_POST['subscriber_id']), false, false, true, true, false)) {
					...
					if (!empty($_POST)) {
						...
						if (!empty($_POST['email'])) {
							...
							elseif ($_POST['email'] != $subscriber -> email && $Subscriber -> email_exists($_POST['email'])) { $errors[] = __('Email address is already in use, try another.', 'wp-mailinglist'); }
						} else {
						...
						if (!empty($_POST['password1'])) {
							...
							elseif ($_POST['password1'] != $_POST['password2']) { $errors['password'] = __('Passwords do not match', 'wp-mailinglist'); }
								...
								$password_hashed = md5($_POST['password1']);
		...
		function ajax_subscribercountdisplay() {
			...
			if (!empty($subscribercount_query)) {
				...
				if ($subscribers = $wpdb -> get_results($subscribercount_query)) {
					...
					<p>
					<?php if ($start > 0) : ?>
						<a href="" class="button button-secondary" onclick="jQuery.colorbox({width:'80%', height:'80%', title:'<?php _e('Selected Subscribers', 'wp-mailinglist'); ?>', href:'<?php echo admin_url('admin-ajax.php?action=newsletters_subscribercountdisplay&start=' . $prevstart); ?>'}); return false;"><i class="fa fa-caret-left"></i> <?php _e('Previous Page', 'wp-mailinglist'); ?></a>
					<?php endif; ?>
					<?php if ($total_subscribers > $end) : ?>
						<a href="" class="button button-secondary" onclick="jQuery.colorbox({width:'80%', height:'80%', title:'<?php _e('Selected Subscribers', 'wp-mailinglist'); ?>', href:'<?php echo admin_url('admin-ajax.php?action=newsletters_subscribercountdisplay&start=' . $nextstart); ?>'}); return false;"><?php _e('Next Page', 'wp-mailinglist'); ?> <i class="fa fa-caret-right"></i></a>
					<?php endif; ?>
					</p>

					<p><a href="" onclick="jQuery.colorbox.close(); return false;" class="button button-primary"><i class="fa fa-times"></i> <?php _e('Close', 'wp-mailinglist'); ?></a></p>
		...
		function ajax_subscribercount($count = true) {
			...
			if (!empty($_POST['mailinglists'])) {
				...
				if (!empty($_POST['daterange']) && $_POST['daterange'] == "Y") {
					if (!empty($_POST['daterangefrom']) && !empty($_POST['daterangeto'])) {
						$daterangefrom = date_i18n("Y-m-d H:i:s", strtotime($_POST['daterangefrom']));
						$daterangeto = date_i18n("Y-m-d H:i:s", strtotime($_POST['daterangeto']));
				...
				if (!empty($_POST['countries'])) {
					if (!empty($_POST['selectedcountries']) && is_array($_POST['selectedcountries'])) {
						$countries = implode("', '", $_POST['selectedcountries']);
			...
			if (!empty($subscribercount)) {
				...
				echo '<p><a href="" onclick="jQuery.colorbox({width:\'80%\', height:\'80%\', title:\'' . __('Selected Subscribers', 'wp-mailinglist') . '\', href:\'' . admin_url('admin-ajax.php?action=newsletters_subscribercountdisplay') . '\'}); return false;"><i class="fa fa-eye"></i> ' . $subscribercount . ' ' . __('emails total', 'wp-mailinglist') . '</a></p>';
			} else {
				echo 0;
			}
				...
				if (!empty($_POST['subscriber_id']) && !empty($_POST['mailinglist_id']) && !empty($_POST['activate'])) {
					...
					if ($subscriber = $Auth -> logged_in()) {
						if ($subscriber -> id == $_POST['subscriber_id']) {
							$Db -> model = $Mailinglist -> model;
							$query = "SELECT * FROM " . $wpdb -> prefix . $Mailinglist -> table . " WHERE `id` = '" . esc_sql($_POST['mailinglist_id']) . "'";
		...
		function paidsubscription_form($subscriber = null, $mailinglist = null, $autosubmit = true, $target = "_self", $extend = false, $pmethod = "paypal") {
			...
			if (!empty($subscriber) && !empty($mailinglist)) {
				if ($pmethod == "2co") {
					...
					$checkoutdata['custom'] = urlencode(json_encode($checkoutdata['custom']));
		...
		function sc_management($atts = array(), $content = null) {
			...
			switch ($method) {
				...
				case 'resubscribe'			:
					...
					break;
				case 'unsubscribe'			:
					global $wpdb, $Html, $Auth, $Db, $Subscriber, $Mailinglist, $SubscribersList;
				...
				case 'loginauth'			:
					...
					$subscriberauth = esc_html($_GET['subscriberauth']);
		...
		function custom_admin_pointers() {
			...
			$version = str_replace(".", "_", $this -> version); // replace all periods in 1.0 with an underscore
			$prefix = 'newsletters_admin_pointers_' . $version . '_';
		...
		function enqueue_scripts() {
			...
			if (is_admin() && !defined('DOING_AJAX')) {
				...
				if ((!empty($_GET['page']) && in_array($_GET['page'], (array) $this -> sections)) || preg_match("/(post\.php|post\-new\.php)/", $_SERVER['REQUEST_URI'], $matches)) {
					...
					if (empty($matches)) {
						wp_enqueue_script('ckeditor', '//cdn.ckeditor.com/4.9.2/full/ckeditor.js', array('jquery'), '4.4.7', false);
				...
				if (!empty($_GET['page']) && in_array($_GET['page'], (array) $this -> sections)) {
					...
					// Ace editor
					if (!empty($_GET['page']) && ($_GET['page'] == $this -> sections -> settings || 
						($_GET['page'] == $this -> sections -> forms))) {
							wp_enqueue_script('ace', '//cdnjs.cloudflare.com/ajax/libs/ace/1.2.6/ace.js', array('jquery'), false, true);
					}
			...
			/* Front-End Scripts */
				...
				if ($captcha_type = $this -> use_captcha()) {
					...
					if (!empty($loadrecaptcha) && !empty($captcha_type) && $captcha_type == "recaptcha") {						
						...
						wp_enqueue_script($handle, 'https://www.google.com/recaptcha/api.js?render=explicit&hl=' . esc_attr(stripslashes($recaptcha_language)), array('jquery'), false, true);
				...
				if ($field = $Field -> get($field_id)) {
					...
					if (!empty($form_id) && !empty($form_field)) {
						...
						if (!empty($fieldset) && $fieldset == true) {
							echo '<div class="form-group' . ((!empty($Subscriber -> errors[$field -> slug])) ? ' has-error' : '') . ' newsletters-fieldholder ' . ((empty($visible) && (!is_admin() || defined('DOING_AJAX'))) ? 'newsletters-fieldholder-hidden hidden' : 'newsletters-fieldholder-visible') . '">';
						}
						...
						if (!empty($fieldset) && $fieldset == true) {
							echo '<div id="form-group' . ((!empty($Subscriber -> errors[$field -> slug])) ? ' has-error' : '') . ' newsletters-' . $optinid . $field -> slug . '-holder" class="' . $col . ' newsletters-fieldholder ' . ((empty($visible) && (!is_admin() || defined('DOING_AJAX'))) ? 'newsletters-fieldholder-hidden hidden' : 'newsletters-fieldholder-visible') . ' ' . $field -> slug . '">';
						}
					...
					switch ($field -> type) {
							...
							if (empty($Subscriber -> data[$field -> slug])) {
								switch ($field -> hidden_type) {
									...
									default  					:
										...
										break;
									case 'get'					:
										$hidden_value = esc_html($_GET[$field -> hidden_value]);
										break;
									case 'global'				:
						...
						case 'file'				:
							...
							<label class="btn btn-default btn-file">
								<?php _e('Browse...', 'wp-mailinglist'); ?> <input onchange="jQuery('#file_upload_<?php echo $field -> id; ?>_info').html(jQuery(this).val().replace('C:\\fakepath\\', ''));" type="file" <?php echo $Html -> tabindex($optinid); ?> name="<?php echo esc_attr(stripslashes($fieldname)); ?>" id="file_upload_<?php echo $field -> id; ?>" />
							</label>
						...
						case 'pre_date'			:
							...
							<input type="text" class="<?php echo $this -> pre; ?>predate <?php echo $this -> pre; ?>text <?php echo $this -> pre; ?> <?php echo $this -> pre; ?>predate<?php echo ((!empty($Subscriber -> errors[$field -> slug])) ? ' newsletters_fielderror' : ''); ?>" value="<?php echo esc_attr(stripslashes($currentDate)); ?>" name="<?php echo $fieldname; ?>" id="<?php echo $this -> pre; ?>-<?php echo $optinid . $field -> slug; ?>" />
						...
						case 'pre_gender'		:
							...
							<select<?php echo $placeholder; ?> class="<?php echo $this -> pre; ?> <?php echo $this -> pre; ?>pregender<?php echo ((!empty($Subscriber -> errors[$field -> slug])) ? ' newsletters_fielderror' : ''); ?>" style="width:auto;" id="<?php echo $this -> pre; ?>-<?php echo $optinid; ?><?php echo $field -> slug; ?>" <?php echo $Html -> tabindex($optinid); ?> name="<?php echo $fieldname; ?>">
								...
								<option <?php echo (!empty($Subscriber -> data[$field -> slug]) && $Subscriber -> data[$field -> slug] == "male") ? 'selected="selected"' : ''; ?> value="male"><?php _e('Male', 'wp-mailinglist'); ?></option>
								<option <?php echo (!empty($Subscriber -> data[$field -> slug]) && $Subscriber -> data[$field -> slug] == "female") ? 'selected="selected"' : ''; ?> value="female"><?php _e('Female', 'wp-mailinglist'); ?></option>
							</select>
					...
					if (!empty($form_id) && !empty($form_field)) {
						...
						if (!empty($caption) && !empty($form_styling['fieldcaptions'])) {
							echo '<p class="help-block">' . stripslashes($caption) . '</p>';
						}
					} else {
						$docaption = false;
			...
			if (!empty($history_id)) {
				...
				if ($email = $this -> History() -> find(array('id' => $history_id))) {
					...
					if (!empty($subscriber)) {
						...
						$querystring = 'newsletters_method=newsletter&id=' . $email -> id . '&user_id=' . $user -> ID . '&authkey=' . $authkey;
						$url = $Html -> retainquery($querystring, home_url());
					}
		...
		function gen_tracking_link($eunique = null) {
			...
			if (!empty($eunique)) {
				if ($this -> get_option('tracking') == "Y") {
					...
					$tracking = '<img alt="' . esc_attr($track_alt) . '" class="newsletters-tracking" src="' . add_query_arg(array($this -> pre . 'method' => "track", 'id' => $eunique), home_url()) . '" />';
				}
			}
	    ...
	    function replace_custom_field($matches = null) {
			...
			if (!empty($matches)) {
				...
				if (!empty($atts['name'])) {
					...
					$atts['name'] = $namesplit[0];
					$defaultvalue = $namesplit[1];
		...
		function admin_notification_import_complete() {	
			...
			if (!empty($import_notification)) {
				...
				$subject = stripslashes($this -> et_subject('import_complete'));
		...
		function admin_notification_queue_complete() {	
			...
			if (!empty($notifyqueuecomplete)) {
				$adminemail = $this -> get_option('adminemail');
				$subject = stripslashes($this -> et_subject('queue_complete'));
			...
			if (!empty($match)) {
				...
				if ($emails = $this -> History() -> find_all(array('sent' => "> 0"), false, array('modified', "DESC"))) {
					...
					foreach ($emails as $email) {
						...
						<h3><a href="<?php echo $Html -> retainquery('newsletters_method=newsletter&id=' . $email -> id, home_url()); ?>" title="<?php echo $email -> subject; ?>"><?php echo $email -> subject; ?></a></h3>
		...
		function execute_mail($subscriber = null, $user = null, $subject = null, $message = null, $attachments = null, $history_id = null, $eunique = null, $shortlinks = true, $emailtype = "newsletter") {
			...
			if (empty($error)) {
				...
				if (!empty($history -> text)) {					
					...
					$altbody = do_shortcode($altbody);
					$altbody = $this -> process_set_variables($subscriber, $user, stripslashes($altbody), $history_id, $eunique);
					$altbody = apply_filters('newsletters_execute_mail_textmessage', $altbody);
					$wpml_textmessage = $altbody;
				}
						...
						switch ($mailapi) {
								...
								try {
									...
									$sparkpost_data = array(
										'content'				=>	array(
											...
											'html'					=>	((empty($newsletters_plaintext)) ? $this -> inlinestyles(apply_filters($this -> pre . '_send_body', stripslashes($message), $phpmailer, $history_id, 'sparkpost', false)) : null),
											//'text'					=>	$altbody,
										),
										'recipients'			=>	array(
											array(
							...
							case 'sendgrid'						:
								...
								if (!empty($attachments) && $attachments != false) {									
									if (is_array($attachments)) {
										foreach ($attachments as $attachment) {
											$file_encoded = base64_encode(file_get_contents($attachment['filename']));
		...
		function ajax_load_new_editor() {
			...
			<div class="postbox" id="contentareabox<?php echo $_REQUEST['contentarea']; ?>">
				<div class="handlediv" title="Click to toggle"><br></div>
					<h3 class="hndle"><span><?php echo __('Content Area', 'wp-mailinglist'); ?> <?php echo $_REQUEST['contentarea']; ?></span></h3>
					<div class="inside">

					<?php

					wp_editor("", 'contentarea' . esc_attr($_REQUEST['contentarea']), array(
						'textarea_name'				=>	'contentarea[' . esc_attr($_REQUEST['contentarea']) . ']',
					));
					...
					<p><a href="" onclick="if (confirm('<?php echo __('Are you sure you want to remove this content area?', 'wp-mailinglist'); ?>')) { deletecontentarea('<?php echo $_REQUEST['contentarea']; ?>', ''); } return false;" class="button button-secondary"><?php _e('Delete', 'wp-mailinglist'); ?></a></p>
				</div>
			</div>
		...
		function updating_plugin() {
			...
			if (version_compare($this -> version, $cur_version) === 1) {
				...
				if (version_compare($cur_version, "4.6.11") < 0) {
					$this -> update_options();
					$this -> check_tables();
					$this -> initialize_default_themes();
		...
		function update_options() {
			...
			$options['scheduleintervalseconds'] = 120;
			$options['emailsperinterval'] = 50;
		...
		function et_message($type = null, $subscriber = null, $language = null, $processvariables = true) {
			$message = false;
		...
		function render($file = null, $params = array(), $output = true, $folder = 'default', $extension = null) {
			...
			if (!empty($file)) {
				...
				echo __('No file was specified for rendering', 'wp-mailinglist');
			}
		}
		...
		function render_email($file = null, $params = array(), $output = false, $html = true, $renderht = true, $theme_id = 0, $shortlinks = true, $fullbody = false) {
			...
			if (!empty($file) || !empty($fullbody)) {
				...
				if (empty($themeintextversion)) {
					global $wpml_textmessage;
					$wpml_textmessage = stripslashes($body);
				}
				...
				if (empty($output) || $output == false) {
					...
					if (!empty($history -> builderon)) {	
						...
						$body = preg_replace("/\[(wpmlcontent|newscontent|newsletters_main_content)\]/si", stripslashes($body), $theme_content);
						...
						$body = do_shortcode(stripslashes($head)) . stripslashes($body) . do_shortcode(stripslashes($foot));
					}
