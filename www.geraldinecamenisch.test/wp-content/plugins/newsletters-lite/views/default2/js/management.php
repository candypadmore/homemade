...
jQuery(document).ready(function() {	
	...
	jQuery('#managementtabs').find('a[href^="#"]').on('click', function(e) {
		e.preventDefault();
...
function wpmlmanagement_activate(subscriber_id, mailinglist_id, activate) {	
	if (activate == "Y") {
		jQuery('#activatelink' + mailinglist_id + ' a').addClass('disabled').html('<i class="fa fa-refresh fa-spin fa-fw"></i> <?php _e('Activating...', 'wp-mailinglist'); ?>');	
	} else {
		jQuery('tr#currentsubscription' + mailinglist_id).fadeOut(1000, function() { jQuery(this).remove(); });
		jQuery('#activatelink' + mailinglist_id + ' a').addClass('disabled').html('<i class="fa fa-refresh fa-spin fa-fw"></i> <?php _e('Removing...', 'wp-mailinglist'); ?>');
	}
...
function wpmlmanagement_subscribe(subscriber_id, mailinglist_id) {
	...
	jQuery('#subscribenowlink' + mailinglist_id + ' a').addClass('disabled').html('<i class="fa fa-refresh fa-spin fa-fw"></i> <?php _e('Subscribing...', 'wp-mailinglist'); ?>');
