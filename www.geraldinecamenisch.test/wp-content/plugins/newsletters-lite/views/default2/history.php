    	...
    	<div class="<?php echo $this -> pre; ?>history_emails">
			<?php foreach ($emails as $email) : ?>
                <div id="<?php echo $this -> pre; ?>history_email<?php echo $email -> id; ?>" class="<?php echo $this -> pre; ?>history_email">
                    <h3 class="<?php echo $this -> pre; ?>history_title"><a href="<?php echo $Html -> retainquery('newsletters_method=newsletter&id=' . $email -> id . '&history=1', home_url()); ?>" title="<?php echo esc_attr(stripslashes($email -> subject)); ?>"><?php echo stripslashes($email -> subject); ?></a></h3>
