...
<div class="wrap <?php echo $this -> pre; ?> <?php echo $this -> sections -> forms; ?> newsletters">
	<?php if (!empty($_GET['id'])) : ?>
		<h1><?php _e('Edit Form', 'wp-mailinglist'); ?> <a onclick="jQuery.colorbox({title:'<?php _e('Create a New Form', 'wp-mailinglist'); ?>', href:'<?php echo admin_url('admin-ajax.php?action=newsletters_forms_createform'); ?>'}); return false;" href="<?php echo admin_url('admin.php?page=' . $this -> sections -> forms . '&method=save'); ?>" class="add-new-h2"><?php _e('Add New', 'wp-mailinglist'); ?></a></h1>
	<?php else : ?>
		<h1><?php _e('Create Form', 'wp-mailinglist'); ?></h1>
	<?php endif; ?>
				...
				<div id="post-body-content">
					<div id="titlediv">
						<div id="titlewrap">
								...
								<div id="title-tabs">
									<ul>
										<?php foreach ($languages as $language) : ?>
											<li><a href="#title-tabs-<?php echo $language; ?>"><?php echo $this -> language_flag($language); ?></a></li>
										<?php endforeach; ?>
									</ul>
									<?php foreach ($languages as $language) : ?>
										<div id="title-tabs-<?php echo $language; ?>">
											<input type="text" class="widefat title" name="title[<?php echo $language; ?>]" value="<?php echo esc_attr(stripslashes($this -> language_use($language, $form -> title))); ?>" id="title_<?php echo $language; ?>" placeholder="<?php echo __('Enter form name here', 'wp-mailinglist'); ?>" />
										</div>
									<?php endforeach; ?>
								</div>
							...
							<?php else : ?>
								<input onclick="jQuery('iframe#content_ifr').attr('tabindex', '2');" tabindex="1" id="title" autocomplete="off" type="text" placeholder="<?php echo esc_attr(stripslashes(__('Enter form name here', 'wp-mailinglist'))); ?>" name="title" value="<?php echo esc_attr(stripslashes($form -> title)); ?>" />
							<?php endif; ?>
						</div>
						...
						<div id="edit-slug-box" class="hide-if-no-js" style="display:<?php echo (!empty($_POST['ishistory'])) ? 'block' : 'none'; ?>;">
							<?php $newsletter_url = $Html -> retainquery('newsletters_method=newsletter&id=' . esc_html($_POST['ishistory']), home_url()); ?>
...
function newsletters_forms_field_add(element, target) {
	...
	var loading = '<div id="newsletters_forms_loading_' + field_id + '" class="newsletters_loading postbox"><h2 class="hndle ui-sortable-handle"><span><i class="fa fa-refresh fa-spin fa-fw"></i> <?php _e('Loading field...', 'wp-mailinglist'); ?></span></h2></div>';
...
jQuery(document).ready(function() {	
	<?php if ($this -> language_do()) : ?>
		newsletters_focus('#title_<?php echo $languages[0]; ?>');
	<?php else : ?>
	...
	<?php if (!empty($manualpostboxes)) : ?>
		<?php foreach ($manualpostboxes as $postbox) : ?>
			...
			<?php if (empty($postbox['mandatory'])) : ?>
				postboxhtml += '<div class="newsletters_delete_handle"><a href="" onclick="if (confirm(\'<?php _e('Are you sure you want to delete this field?', 'wp-mailinglist'); ?>\')) { newsletters_forms_field_delete(\'<?php echo $postbox['field_id']; ?>\'); } return false;"><i class="fa fa-times fa-fw"></i></a></div>';
		...
		<?php endforeach; ?>
	<?php else : ?>
		jQuery('#normal-sortables').append('<div class="temp-placeholder" style="width:auto; height:auto;"><p><i class="fa fa-reply"></i> <?php _e('Drag fields here to add them to the form', 'wp-mailinglist'); ?></p></div>');
	<?php endif; ?>
