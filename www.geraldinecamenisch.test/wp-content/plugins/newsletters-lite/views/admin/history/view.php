...
<div class="wrap newsletters <?php echo $this -> pre; ?> newsletters">
	            ...
	            <tr class="<?php echo $class = (empty($class)) ? 'alternate' : ''; ?>">
	                ...
	                <td>
	                    ...
	                    <?php if (!empty($history -> theme_id) && $theme = $Db -> find(array('id' => $history -> theme_id))) : ?>
	                    	...
	                    	<a href="" onclick="jQuery.colorbox({title:'<?php echo sprintf(__('Edit Template: %s', 'wp-mailinglist'), __($theme -> title)); ?>', href:newsletters_ajaxurl + 'action=newsletters_themeedit&amp;id=<?php echo $theme -> id; ?>'}); return false;" class=""><i class="fa fa-pencil fa-fw"></i></a>
	                    <?php else : ?>
	                    	<?php _e('None', 'wp-mailinglist'); ?>
	                    <?php endif; ?>
	                </td>
    ...
    <!-- History Preview -->
    <h3><?php _e('Preview', 'wp-mailinglist'); ?> <a href="<?php echo admin_url('admin-ajax.php?action=' . $this -> pre . 'history_iframe&id=' . $history -> id); ?>" target="_blank" class="add-new-h2"><?php _e('Open in New Window', 'wp-mailinglist'); ?></a></h3>
