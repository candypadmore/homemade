...
<div class="wrap newsletters <?php echo $this -> pre; ?>">
	<h1><?php _e('Save a Custom Field', 'wp-mailinglist'); ?></h1>
	...
	<form action="?page=<?php echo $this -> sections -> fields; ?>&amp;method=save" method="post" id="Field.saveform">
		...
		<table class="form-table">
			<tbody>
				<tr>
					<th><label for="Field_title"><?php _e('Title', 'wp-mailinglist'); ?></label></th>
					<td>
							...
							<div id="tabs_title">
								...
								<?php foreach ($languages as $language) : ?>
									<div id="tabs_title_<?php echo $tabs_title; ?>">
										<input placeholder="<?php echo esc_attr(stripslashes(__('Enter field title/name here', 'wp-mailinglist'))); ?>" <?php echo ($tabs_title == 1) ? 'onkeyup="wpml_titletoslug(this.value);"' : ''; ?> type="text" class="widefat" name="Field[title][<?php echo $language; ?>]" value="<?php echo esc_attr(stripslashes($Html -> field_value('Field[title]', $language))); ?>" id="Field_title_<?php echo $language; ?>" />
									</div>
									<?php $tabs_title++; ?>
								<?php endforeach; ?>
							</div>
						<?php else : ?>
							<input placeholder="<?php echo esc_attr(stripslashes(__('Enter field title/name here', 'wp-mailinglist'))); ?>" onkeyup="wpml_titletoslug(this.value);" type="text" class="widefat" name="Field[title]" value="<?php echo esc_attr(stripslashes($Html -> field_value('Field[title]'))); ?>" id="Field_title" />
						<?php endif; ?>
                        <span class="howto"><?php _e('Title/name for this custom field as it will be displayed on subscribe forms.', 'wp-mailinglist'); ?></span>
                        <?php echo $Html -> field_error('Field[title]'); ?>
                    </td>
				</tr>
				<?php if (empty($slug) || ($slug != 'email' && $slug != "list")) : ?>
	                <tr>
	                	<th><label for="Field_slug"><?php _e('Slug/Nicename', 'wp-mailinglist'); ?></label></th>
	                    <td>
	                    	<input placeholder="<?php echo esc_attr(stripslashes(__('Enter field slug for database and shortcodes use here', 'wp-mailinglist'))); ?>" id="Field_slug" type="text" class="widefat" name="Field[slug]" value="<?php echo $Html -> field_value('Field[slug]'); ?>" />
	                        ...
	                        <?php endif; ?>
	                        <span class="howto"><?php _e('Only use letters, lowercase, no spaces or other characters, please.', 'wp-mailinglist'); ?></span>
	                    </td>
	                </tr>
	            <?php else : ?>
	            ...
	            <?php endif; ?>
				<tr>
					<th><label for="Field.caption"><?php _e('Caption/Description', 'wp-mailinglist'); ?></label></th>
					<td>
						<?php if ($this -> language_do()) : ?>
							<div id="tabs_caption">
								<?php $tabs_caption = 1; ?>
								...
								<?php foreach ($languages as $language) : ?>
									<div id="tabs_caption_<?php echo $tabs_caption; ?>">
										...
										wp_editor(stripslashes($Html -> field_value('Field[caption]', $language)), 'Field_caption_' . $language, $settings); 
						...
						<?php else : ?>
							...
							wp_editor(stripslashes($Html -> field_value('Field[caption]')), 'Field_caption', $settings); 
							
							?>
						<?php endif; ?>
						<span class="howto"><small><?php _e('(optional)', 'wp-mailinglist'); ?></small> <?php _e('Give this field a descriptive caption/notation for subscribers to see.', 'wp-mailinglist'); ?></span>
					</td>
				</tr>
				<tr>
					<th><label for="Field.watermark"><?php _e('Watermark', 'wp-mailinglist'); ?></label></th>
					<td>
						<?php if ($this -> language_do()) : ?>
							<div id="tabs_watermark">
								...
								<?php foreach ($languages as $language) : ?>
									<div id="tabs_watermark_<?php echo $tabs_watermark; ?>">
										<input placeholder="<?php echo esc_attr(stripslashes(__('Enter watermark text here', 'wp-mailinglist'))); ?>" type="text" class="widefat" name="Field[watermark][<?php echo $language; ?>]" value="<?php echo esc_attr(stripslashes($Html -> field_value('Field[watermark]', $language))); ?>" id="Field_watermark_<?php echo $language; ?>" />
									</div>
									<?php $tabs_watermark++; ?>
								<?php endforeach; ?>
							</div>
		...
		<?php if (empty($slug) || ($slug != 'email' && $slug != "list")) : ?>
			<table class="form-table">
				<tbody>
					<tr>
						<th><label for="Field.type"><?php _e('Field Type', 'wp-mailinglist'); ?></label></th>
						<td>
							<?php 
		...
		<div id="hiddendiv" style="display:<?php echo ($Html -> field_value('Field[type]') == "hidden") ? 'block' : 'none'; ?>;">
			<table class="form-table">
				<tbody>
					<tr>
						<th><label for=""><?php _e('Hidden Value', 'wp-mailinglist'); ?></label>
		...
		<div id="typediv" style="display:<?php echo ($Html -> field_value('Field[type]') == "checkbox" || $Html -> field_value('Field[type]') == "radio" || $Html -> field_value('Field[type]') == "select") ? 'block' : 'none'; ?>;">
			<table class="form-table">
				<tbody>
					<tr>
						<th><label for="Field.fieldoptions"><?php _e('Field Options', 'wp-mailinglist'); ?></label></th>
						<td>	
							...
							<p class="submit">
								<a href="" class="button" onclick="newsletters_fieldoptions_add(); return false;"><i class="fa fa-plus-circle"></i> <?php _e('Add Option', 'wp-mailinglist'); ?></a>
							</p>
							...
							var newsletters_fieldoptions_remove = function(element) {
								if (confirm('<?php _e('Are you sure you want to delete this option?', 'wp-mailinglist'); ?>')) {
									...
									if (id != "") {
										jQuery.ajax({
											...
											alert('<?php _e('Failed, please try again', 'wp-mailinglist'); ?>');
										});
									} else {
										jQuery(element).closest('li').remove();
									}
		...
		<?php if (empty($slug) || ($slug != 'email' && $slug != "list")) : ?>
			<table class="form-table">
				<tbody>
					<tr>
						<th><label for="Field.requiredNo"><?php _e('Required', 'wp-mailinglist'); ?></label></th>
						<td>
		...
		<?php if (empty($slug) || ($slug != 'email' && $slug != "list")) : ?>
			...
			<table class="form-table">
				<tbody>
					<tr>
						<th><label for="Field_display_always"><?php _e('Display', 'wp-mailinglist'); ?></label></th>
						<td>
							<?php $display = $Html -> field_value('Field[display]'); ?>
							<label><input onclick="jQuery('#displaydiv').hide();" <?php echo (empty($display) || $display == "always") ? 'checked="checked"' : ''; ?> type="radio" name="Field[display]" value="always" id="Field_display_always" /> <?php _e('Always show', 'wp-mailinglist'); ?></label>
							<label><input onclick="jQuery('#displaydiv').show();" <?php echo (!empty($display) && $display == "specific") ? 'checked="checked"' : ''; ?> type="radio" name="Field[display]" value="specific" id="Field_display_specific" /> <?php _e('Specific list(s)', 'wp-mailinglist'); ?></label>
							<span class="howto"><?php _e('Should this field always show or only for specific mailing list/s?', 'wp-mailinglist'); ?></span>
			...
			<div id="displaydiv" style="display:<?php echo ($Html -> field_value('Field[display]') == "specific") ? 'block' : 'none'; ?>;">
				<table class="form-table">
					<tbody>
						<tr>
							<th><label for="checkboxall"><?php _e('Mailing Lists', 'wp-mailinglist'); ?></label></th>
							<td>
								<?php $lists = $Mailinglist -> select(true); ?>
								<?php if (!empty($lists)) : ?>
									<label style="font-weight:bold;"><input type="checkbox" id="mailinglistsselectall" name="mailinglistsselectall" value="1" onclick="jqCheckAll(this, 'Field.saveform', 'Field[mailinglists]');" /> <?php _e('Select All', 'wp-mailinglist'); ?></label><br/>
									<div id="newsletters-mailinglists-checkboxes" class="scroll-list">
										<?php foreach ($lists as $id => $title) : ?>
											...
											$fieldslist = (empty($Field -> data -> id)) ? esc_html($_POST[$Field -> model]['mailinglists']) : $checkedlistsbyfield; 
								...
								<?php else : ?>
									<p class="newsletters_error"><?php _e('No mailing lists were found', 'wp-mailinglist'); ?></p>
								<?php endif; ?>
							</td>
						</tr>
					</tbody>
...
jQuery(document).ready(function() {
	...
	<?php if ($this -> language_do()) : ?>
		...
		newsletters_focus('#Field_title_<?php echo $languages[0]; ?>');
	<?php else : ?>
