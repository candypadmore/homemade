...
<div class="wrap newsletters <?php echo $this -> pre; ?>">
	...
	<form id="newsletters-subscriber-form" name="optinform<?php echo $subscriber -> id; ?>" action="?page=<?php echo $this -> sections -> subscribers; ?>&amp;method=save" method="post" enctype="multipart/form-data">
		...
		<table class="form-table">
			<tbody>
				<tr>
					<th><label for="Subscriber.email"><?php _e('Email Address', 'wp-mailinglist'); ?></label>
					...
					<td>
						...
						<span class="howto"><?php _e('Valid email address of the subscriber to receive newsletters.', 'wp-mailinglist'); ?></span>
					</td>
				</tr>
				<tr>
					<th><label for="checkboxall"><?php _e('Mailing List/s', 'wp-mailinglist'); ?><label>
					...
					<td>
						<?php if ($mailinglists = $Mailinglist -> select(true)) : ?>						
							<div><label style="font-weight:bold;"><input type="checkbox" name="checkboxall" value="checkboxall" id="checkboxall" onclick="jqCheckAll(this, 'newsletters-subscriber-form', 'Subscriber[mailinglists]');" /> <?php _e('Select all', 'wp-mailinglist'); ?></label></div>
							<div class="scroll-list" id="newsletters-mailinglists-checkboxes">
								<?php foreach ($mailinglists as $list_id => $list_title) : ?>
									...
									<label><input onclick="jQuery('#mailinglist_<?php echo $list_id; ?>_expiration').toggle();" <?php echo (!empty($Subscriber -> data -> mailinglists) && in_array($list_id, $Subscriber -> data -> mailinglists)) ? 'checked="checked"' : ''; ?> type="checkbox" class="Mailinglist_checklist" name="Subscriber[mailinglists][]" value="<?php echo $list_id; ?>" id="Subscriber_mailinglists_<?php echo $list_id; ?>" /> <?php echo __($list_title); ?> (<?php echo $SubscribersList -> count(array('list_id' => $list_id)); ?> <?php _e('subscribers', 'wp-mailinglist'); ?>)</label><br/>
						...
						<span class="howto"><?php _e('All ticked/checked subscriptions are activated immediately.', 'wp-mailinglist'); ?></span>													
					</td>
				</tr>
				<tr>
				...
				<tr>
					<th><?php _e('Register as WordPress user?', 'wp-mailinglist'); ?>
		...
		<div id="registereddiv" style="display:<?php echo ($Html -> field_value('Subscriber[registered]') == "Y") ? 'block' : 'none'; ?>;">
			<table class="form-table">
				<tbody>
					<tr>
						<th><?php _e('Wordpress Username', 'wp-mailinglist'); ?></th>
		...
		<table class="form-table">
			<tbody>
				<tr>
					<th><?php _e('Email Format', 'wp-mailinglist'); ?>
        ...
        <?php if (!empty($fields)) : ?>
			<br/>
			<h3><?php _e('Custom Fields', 'wp-mailinglist'); ?> (<?php echo $Html -> link(__('show/hide', 'wp-mailinglist'), '#void', array('onclick' => "jQuery('#customfieldsdiv').toggle();")); ?>)
		...
		<table class="form-table">
			<tbody>
				<tr>
					<th><label for="Subscriber_mandatory_N"><?php _e('Mandatory?', 'wp-mailinglist'); ?></label>
					...
					<td>
						<label><input <?php echo (!empty($mandatory) && $mandatory == "Y") ? 'checked="checked"' : ''; ?> type="radio" name="Subscriber[mandatory]" value="Y" id="Subscriber_mandatory_Y" /> <?php _e('Yes', 'wp-mailinglist'); ?></label>
						<label><input <?php echo (empty($mandatory) || (!empty($mandatory) && $mandatory == "N")) ? 'checked="checked"' : ''; ?> type="radio" name="Subscriber[mandatory]" value="N" id="Subscriber_mandatory_N" /> <?php _e('No', 'wp-mailinglist'); ?></label>
						<span class="howto"><?php _e('A mandatory subscriber cannot unsubscribe', 'wp-mailinglist'); ?></span>
					</td>
				</tr>
			</tbody>
		</table>
