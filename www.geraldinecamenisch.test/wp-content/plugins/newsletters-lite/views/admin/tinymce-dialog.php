...
<html xmlns="https://www.w3.org/1999/xhtml">
	<head>
		<title><?php _e('Newsletter Functions', 'wp-mailinglist'); ?></title>
		...
		function insertTag() {
			...
			if (postspanel.className.indexOf('current') != -1) {
				...
				if (posts_insert == "single") {
					...
					if (posts_single_id = jQuery('#posts_post_menu').val()) {
						...
						alert('<?php _e('Please select a post from the menu.', 'wp-mailinglist'); ?>');	
					}
			...
			if (templatepanel.className.indexOf('current') != -1) {
				...
				if (!template_id) {
					alert('<?php echo __('Choose a snippet', 'wp-mailinglist'); ?>');	
				} else {
		...
		function posts_changeCategory() {			
			...
			if (jQuery('input[name="postslanguage"]').length > 0) {				
				...
				if (postslanguage == "" || postslanguage == "undefined" || postslanguage == undefined) {
					alert('<?php _e('Please choose a language.', 'wp-mailinglist'); ?>');
					return false;
				}
			}
	...
	<body onload="init(); document.body.style.display = '';">	
		<form onsubmit="insertTag(); return false;" action="#">
        	<div class="tabs">
				<ul>
					<li id="subscription_tab" class="current"><span><a href="javascript:mcTabs.displayTab('subscription_tab','subscription_panel');" onmousedown="return false;"><?php _e('Subscription Form', 'wp-mailinglist'); ?></a></span></li>
					<li id="posts_tab"><span><a href="javascript:mcTabs.displayTab('posts_tab','posts_panel');" onmousedown="return false;"><?php _e('Posts', 'wp-mailinglist'); ?></a></span></li>
					<li id="thumbnail_tab"><span><a href="javascript:mcTabs.displayTab('thumbnail_tab','thumbnail_panel');" onmousedown="return false;"><?php _e('Thumbnail', 'wp-mailinglist'); ?></a></span></li>
					<li id="history_tab"><span><a href="javascript:mcTabs.displayTab('history_tab','history_panel');" onmousedown="return false;"><?php _e('Email History', 'wp-mailinglist'); ?></a></span></li>
					<li id="template_tab"><span><a href="javascript:mcTabs.displayTab('template_tab','template_panel');" onmousedown="return false;"><?php _e('Template', 'wp-mailinglist'); ?></a></span></li>
				</ul>
			</div>
			...
			<div class="panel_wrapper">
				<div id="subscription_panel" class="panel current">
                    ...
                    <p><?php _e('Choose to which list(s) subscribers will be subscribed to.', 'wp-mailinglist'); ?></p>
                    
					<table cellpadding="4" cellspacing="4" border="0">
						<tbody>
							<tr>
								<td nowrap="nowrap" valign="top"><label for="list"><?php _e('Mailing List', 'wp-mailinglist'); ?>:</label></td>
								<td>                                															
									<select onchange="if (this.value == 'select' || this.value == 'checkboxes') { jQuery('#listsdiv').show(); } else { jQuery('#listsdiv').hide(); }" class="widefat" style="width:210px;" name="list" id="list">
										...
										<?php if ($lists = $Mailinglist -> select($private = true)) : ?>
											<optgroup label="<?php _e('Specific Mailing List', 'wp-mailinglist'); ?>">
                                    ...
                                    <br/><small><?php _e('You can allow multiple choices or a specific mailinglist.', 'wp-mailinglist'); ?></small>
								</td>
							</tr>
							<tr id="listsdiv">
								<td nowrap="nowrap" valign="top"><label for="lists"><?php _e('Lists:', 'wp-mailinglist'); ?></label></td>
								<td>
									<input type="text" name="lists" value="" id="lists" />
									<br/><small><?php _e('Comma separated list IDs to show (eg. 11,3,7), leave empty for all.', 'wp-mailinglist'); ?></small>
								</td>
							</tr>
						</tbody>
					</table>
                ...
                <div id="posts_panel" class="panel">
                    ...
                    <table cellpadding="4" cellspacing="4" border="0">
                    	<tbody>
                        	<?php if ($this -> language_do()) : ?>
                                <tr>
                                    <td nowrap="nowrap" valign="top"><label for=""><?php _e('Language:', 'wp-mailinglist'); ?></label></td>
                                    <td>
                            ...
                            <?php endif; ?>
                        	<tr>
                            	<td nowrap="nowrap" valign="top"><label for="posts_insert_single"><?php _e('Insert:', 'wp-mailinglist'); ?></label></td>
                                <td>
                                	<label><input onclick="jQuery('#posts_insert_single_div').show(); jQuery('#posts_insert_multiple_div').hide();" type="radio" name="posts_insert" value="single" id="posts_insert_single" /> <?php _e('Single Post', 'wp-mailinglist'); ?></label>
                                    <label><input checked="checked" onclick="jQuery('#posts_insert_single_div').hide(); jQuery('#posts_insert_multiple_div').show();" type="radio" name="posts_insert" value="multiple" id="posts_insert_multiple" /> <?php _e('Multiple Posts', 'wp-mailinglist'); ?></label>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <div id="posts_insert_single_div" style="display:none;">
                    	<table cellpadding="4" cellspacing="4" border="0">
                        	<tbody>
                        		<tr>
                                	<td nowrap="nowrap" valign="top"><label for="post_showdate_Y"><?php _e('Show Date:', 'wp-mailinglist'); ?></label></td>
                                	<td>
                                		<label><input checked="checked" type="radio" name="post_showdate" value="Y" id="post_showdate_Y" /> <?php _e('Yes', 'wp-mailinglist'); ?></label>
                                		<label><input type="radio" name="post_showdate" value="N" id="post_showdate_N" /> <?php _e('No', 'wp-mailinglist'); ?></label>
                                		<br/><small><?php _e('Show the posted date for this post?', 'wp-mailinglist'); ?></small>
                                	</td>
                                </tr>
                                <tr>
                                	<td nowrap="nowrap" valign="top"><label for="post_eftype_excerpt"><?php _e('Full/Excerpt:', 'wp-mailinglist'); ?></label></td>
                                	<td>
                                		<label><input type="radio" name="post_eftype" value="full" id="post_eftype_full" /> <?php _e('Full', 'wp-mailinglist'); ?></label>
                                		<label><input checked="checked" type="radio" name="post_eftype" value="excerpt" id="post_eftype_excerpt" /> <?php _e('Excerpt', 'wp-mailinglist'); ?></label>
                                		<br/><small><?php _e('Insert a full post or the excerpt of a post.', 'wp-mailinglist'); ?></small>
                                	</td>
                                </tr>
                            	<tr>
                                	<td nowrap="nowrap" valign="top"><label for="posts_category_menu"><?php _e('Category:', 'wp-mailinglist'); ?></label></td>
                                    <td>
                                ...
                                <?php if ($post_types = $this -> get_custom_post_types()) : ?>
                                	<tr>
                                		<td nowrap="nowrap" valign="top"><label for=""><?php _e('Custom Post Types:', 'wp-mailinglist'); ?></label></td>
                                		<td>
                                			...
                                			<?php endforeach; ?>
                                			<small><?php _e('Choose the custom post types to take posts from.', 'wp-mailinglist'); ?></small>
                                		</td>
                                	</tr>
                                <?php endif; ?>
                                <tr>
                                	<td nowrap="nowrap" valign="top"><label for="posts_post_menu"><?php _e('Choose Post:', 'wp-mailinglist'); ?></label></td>
                                    <td>
                    ...
                    <div id="posts_insert_multiple_div" style="display:block;">
                    	<table cellpadding="4" cellspacing="4" border="0">
                        	<tbody>
                            	<tr>
                                	<td nowrap="nowrap" valign="top"><label for="posts_number"><?php _e('Number of Posts:', 'wp-mailinglist'); ?></label></td>
                                    <td>
                                    	<input style="width:45px;" type="text" name="posts_number" value="10" id="posts_number" />
                                    </td>
                                </tr>
                                <tr>
                                	<td nowrap="nowrap" valign="top"><label for="posts_showdate_Y"><?php _e('Show Date', 'wp-mailinglist'); ?></label></td>
                                	<td>
                                		<label><input checked="checked" type="radio" name="posts_showdate" value="Y" id="posts_showdate_Y" /> <?php _e('Yes', 'wp-mailinglist'); ?></label>
                                		<label><input type="radio" name="posts_showdate" value="N" id="posts_showdate_N" /> <?php _e('No', 'wp-mailinglist'); ?></label>
                                		<br/><small><?php _e('Show the posted date of each post?', 'wp-mailinglist'); ?></small>
                                	</td>
                                </tr>
                                <tr>
                                	<td nowrap="nowrap" valign="top"><label for="posts_eftype_excerpt"><?php _e('Full/Excerpt:', 'wp-mailinglist'); ?></label></td>
                                	<td>
                                		<label><input type="radio" name="posts_eftype" value="full" id="posts_eftype_full" /> <?php _e('Full', 'wp-mailinglist'); ?></label>
                                		<label><input checked="checked" type="radio" name="posts_eftype" value="excerpt" id="posts_eftype_excerpt" /> <?php _e('Excerpt', 'wp-mailinglist'); ?></label>
                                		<br/><small><?php _e('Insert full posts or the excerpt of posts.', 'wp-mailinglist'); ?></small>
                                	</td>
                                </tr>
                                <tr>
                                	<td nowrap="nowrap" valign="top"><label for="posts_orderby"><?php _e('Order By:', 'wp-mailinglist'); ?></label></td>
                                    <td>
                                    	<select name="posts_orderby" id="posts_orderby" style="width:210px;">
                                        	<option value="post_date"><?php _e('Date', 'wp-mailinglist'); ?></option>
                                            <option value="author"><?php _e('Author', 'wp-mailinglist'); ?></option>
                                            <option value="category"><?php _e('Category', 'wp-mailinglist'); ?></option>
                                            <option value="content"><?php _e('Post Content', 'wp-mailinglist'); ?></option>
                                            <option value="ID"><?php _e('Post ID', 'wp-mailinglist'); ?></option>
                                            <option value="menu_order"><?php _e('Menu Order', 'wp-mailinglist'); ?></option>
                                            <option value="title"><?php _e('Post Title', 'wp-mailinglist'); ?></option>
                                            <option value="rand"><?php _e('Random Order', 'wp-mailinglist'); ?></option>
                                        </select>
                                    </td>
                                </tr>
                                <tr>
                                	<td nowrap="nowrap" valign="top"><label for="posts_order"><?php _e('Order Direction:', 'wp-mailinglist'); ?></label></td>
                                    <td>
                                    	<select style="width:210px;" name="posts_order" id="posts_order">
                                        	<option value="DESC"><?php _e('Descending (new to old/Z to A/Large to Small)', 'wp-mailinglist'); ?></option>
                                        	<option value="ASC"><?php _e('Ascending (old to new/A to Z/Small to Large)', 'wp-mailinglist'); ?></option>
                                        </select>
                                    </td>
                                </tr>
                            	<tr>
                                	<td nowrap="nowrap" valign="top"><label for="posts_categories"><?php _e('Posts Category:', 'wp-mailinglist'); ?></label></td>
                                    <td>
                                ...
                                <?php if ($post_types = $this -> get_custom_post_types()) : ?>
                                	<tr>
                                		...
                                		<td>
                                			...
                                			<?php endforeach; ?>
                                			<small class="howto"><?php _e('Choose the custom post types to take posts from.', 'wp-mailinglist'); ?></small>
                                		</td>
                                	</tr>
                                <?php endif; ?>
                            </tbody>
                		...
                		<tbody>
                			<tr>
                				<td nowrap="nowrap" valign="top">
                					<label for="thumbnail_post_id"><?php _e('Post ID:', 'wp-mailinglist'); ?></label>
                				</td>
                				<td>
                					<input type="text" name="thumbnail_post_id" value="" id="thumbnail_post_id" />
                					<br/>
                					<small class="howto"><?php _e('Featured image post ID. Leave empty for current post.', 'wp-mailinglist'); ?></small>
                				</td>
                			</tr>
                			<tr>
                				<td nowrap="nowrap" valign="top">
                					<label for="thumbnail_size"><?php _e('Size:', 'wp-mailinglist'); ?></label>
                				</td>
                				<td>
                					...
                					<br/>
                					<small class="howto"><?php _e('Preferred size of the image.', 'wp-mailinglist'); ?>
                				</td>
                			</tr>
                		</tbody>
                	</table>
                ...
                <div id="history_panel" class="panel">
                	...
                	<table cellpadding="4" cellspacing="4" border="0">
                    	<tbody>
                        	<tr>
                            	<td nowrap="nowrap" valign="top"><label for="history_number"><?php _e('Number of Emails:', 'wp-mailinglist'); ?></label></td>
                                <td>
                                	<input style="width:45px;" type="text" name="history_number" value="" id="history_number" /><br/>
                                    <span class="howto"><?php _e('Leave empty for unlimited/all emails.', 'wp-mailinglist'); ?></span>
                                </td>
                            </tr>
                            <tr>
                            	<td nowrap="nowrap" valign="top"><label for="history_orderby"><?php _e('Order By:', 'wp-mailinglist'); ?></label></td>
                                <td>
                                	<select name="history_orderby" id="history_orderby" style="width:210px;">
                                    	<option value="modified"><?php _e('Date', 'wp-mailinglist'); ?></option>
                                        <option value="subject"><?php _e('Email Subject', 'wp-mailinglist'); ?></option>
                                        <option value="sent"><?php _e('Times Sent', 'wp-mailinglist'); ?></option>
                                    </select>
                                </td>
                            </tr>
                            <tr>
                            	<td nowrap="nowrap" valign="top"><label for="history_order"><?php _e('Order Direction:', 'wp-mailinglist'); ?></label></td>
                                <td>
                            ...
                            <tr>
                            	<td nowrap="nowrap" valign="top"><label for="history_lists"><?php _e('Mailing List/s:', 'wp-mailinglist'); ?></label></td>
                                <td>
                                	<select name="history_lists" id="history_lists" multiple="multiple">
                                    	<option value=""><?php _e('- All Mailing Lists -', 'wp-mailinglist'); ?></option>
                                    ...
                                    <br/><span class="howto"><?php _e('You can select multiple mailing lists by holding CTRL key.', 'wp-mailinglist'); ?></span>
                                </td>
                            </tr>
                        </tbody>
                    </table>
				...
				<div id="template_panel" class="panel">
					...
					<table cellpadding="4" cellspacing="4" border="0">
						<tbody>
							<tr>
								<td nowrap="nowrap" valign="top"><label for="template"><?php _e('Snippet:', 'wp-mailinglist'); ?></label></td>
								<td>
									<?php
									...
									<?php if (!empty($templates)) : ?>
										<select name="template" id="template">
											<option value=""><?php _e('- Select Template -', 'wp-mailinglist'); ?></option>
