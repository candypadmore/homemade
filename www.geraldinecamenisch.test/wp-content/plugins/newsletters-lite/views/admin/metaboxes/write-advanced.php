...
$postmailinglists = (empty($_POST['newsletters_mailinglists'])) ? get_post_meta($post_id, '_newsletters_mailinglists', true) : esc_html($_POST['newsletters_mailinglists']);
$theme_id = (empty($_POST['newsletters_theme_id'])) ? get_post_meta($post_id, '_newsletters_theme_id', true) : esc_html($_POST['newsletters_theme_id']);
$language = (empty($_POST['newsletters_language'])) ? get_post_meta($post_id, '_newsletters_language', true) : esc_html($_POST['newsletters_language']);
$sendonpublishef = (empty($_POST['newsletters_sendonpublishef'])) ? get_post_meta($post_id, '_newsletters_sendonpublishef', true) : esc_html($_POST['newsletters_sendonpublishef']);
		    ...
		    <?php if ($languages = $this -> language_getlanguages()) : ?>
		    	<p>
					<?php foreach ($languages as $lang) : ?>
		                <label><input <?php echo ((!empty($language) && $language == $lang) || ($this -> language_default() == $lang)) ? 'checked="checked"' : ''; ?> type="radio" name="newsletters_language" value="<?php echo $lang; ?>" id="newsletters_language_<?php echo $lang; ?>" /> <?php echo $this -> language_flag($lang); ?> <?php echo stripslashes($this -> language_name($lang)); ?></label><br/>
		            <?php endforeach; ?>
		        </p>
		    <?php else : ?>
			...
			<table class="form-table">
				<tbody>
					<tr>
						...
						<td>
							<input type="text" class="widefat" name="newsletters_subject" value="<?php echo esc_attr(stripslashes($newsletters_subject)); ?>" id="newsletters_subject" />
					    ...
					    <?php foreach ($themes as $theme) : ?>
					       <tr class="<?php echo $class = (empty($class)) ? 'alternate' : ''; ?>">
						        ...
						        <td>
									...
									<a href="" onclick="jQuery.colorbox({title:'<?php echo sprintf(__('Edit Template: %s', 'wp-mailinglist'), $theme -> title); ?>', href:newsletters_ajaxurl + 'action=newsletters_themeedit&amp;id=<?php echo $theme -> id; ?>'}); return false;" class=""><i class="fa fa-pencil fa-fw"></i></a>
						        </td>
					        </tr>
					    <?php endforeach; ?>
							</tbody>
