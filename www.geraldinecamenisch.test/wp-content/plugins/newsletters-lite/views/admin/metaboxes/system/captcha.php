...
<table class="form-table">
	<tbody>
		<tr>
			<th><label for="captcha_type_none"><?php _e('Captcha Type', 'wp-mailinglist'); ?></label>
			...
			<td>
				<label><input onclick="jQuery('#recaptcha_div').hide(); jQuery('#rsc_div').hide();" <?php echo (empty($captcha_type) || $captcha_type == "none") ? 'checked="checked"' : ''; ?> type="radio" name="captcha_type" value="none" id="captcha_type_none" /> <?php _e('None', 'wp-mailinglist'); ?></label>
				<label><input onclick="jQuery('#recaptcha_div').show(); jQuery('#rsc_div').hide();" <?php echo (!empty($captcha_type) && $captcha_type == "recaptcha") ? 'checked="checked"' : ''; ?> type="radio" name="captcha_type" value="recaptcha" id="captcha_type_recaptcha" /> <?php _e('reCAPTCHA', 'wp-mailinglist'); ?></label>
				<label><input onclick="jQuery('#recaptcha_div').hide(); jQuery('#rsc_div').show();" <?php echo (!empty($captcha_type) && $captcha_type == "rsc") ? 'checked="checked"' : ''; ?> type="radio" name="captcha_type" value="rsc" id="captcha_type_rsc" <?php echo (!$this -> is_plugin_active('captcha')) ? 'disabled="disabled"' : ''; ?> /> <?php _e('Really Simple Captcha', 'wp-mailinglist'); ?></label>
				...
				<span class="plugin-install-rsc">
					<?php if (!$this -> is_plugin_active('captcha', true)) : ?>
						<button type="button" class="install-now button button-secondary" href="<?php echo wp_nonce_url(admin_url('plugin-install.php?tab=plugin-information&plugin=really-simple-captcha&TB_iframe=true&width=600&height=550')); ?>">
							<i class="fa fa-check fa-fw"></i> <?php _e('Install RSC', $this -> plugin_name); ?>
						</button>
					<?php elseif (!$this -> is_plugin_active('captcha', false)) : ?>
						<a class="button button-secondary" href="<?php echo wp_nonce_url(admin_url('plugins.php?action=activate&plugin=' . $path), 'activate-plugin_' . $path); ?>" class=""><i class="fa fa-check fa-fw"></i> <?php _e('Activate RSC', 'wp-mailinglist'); ?></a>
					<?php endif; ?>
				</span>
				
				<span class="howto"><?php _e('Choose the type of captcha you want to use as a security image on subscribe forms or turn off by choosing "None"', 'wp-mailinglist'); ?></span>
			</td>
		</tr>
	</tbody>
</table>
...
(function($) {
	...
	$plugininstall.on('click', '.install-now', function() {
		tb_show('<?php _e('Install Really Simple Captcha Plugin', 'wp-mailinglist'); ?>', $(this).attr('href'), false);
		return false;
	});
	...
	$document.on('wp-plugin-installing', function(event, args) {
		$plugininstall.find('.install-now').html('<i class="fa fa-refresh fa-spin fa-fw"></i> <?php echo __('Installing', 'wp-mailinglist'); ?>').prop('disabled', true);
	});
	
	$document.on('wp-plugin-install-success', function(event, response) {			
		$plugininstall.find('.install-now').html('<i class="fa fa-check fa-fw"></i> <?php _e('Activate RSC', 'wp-mailinglist'); ?>').attr('href', response.activateUrl).prop('disabled', false);
	...
	$document.on('wp-plugin-install-error', function(event, response) {
		...
		$plugininstall.find('.install-now').html('<i class="fa fa-check fa-fw"></i> <?php echo __('Install RSC', 'wp-mailinglist'); ?>').prop('disabled', false);
	});
})(jQuery);
</script>
	...
	<table class="form-table">
		<tbody>
			<tr>
				<th>&nbsp;</th>
				<td>
					<p><?php echo sprintf(__('In order to use reCAPTCHA, the public and private keys below are required.<br/>Go to the reCAPTCHA sign up and %screate a set of keys%s for this domain.', 'wp-mailinglist'), '<a href="https://www.google.com/recaptcha/admin/create" onclick="jQuery.colorbox({iframe:true, fastIframe:false, width:\'80%\', height:\'80%\', href:\'https://www.google.com/recaptcha/admin/create\'}); return false;">', '</a>'); ?></p>
				</td>
			</tr>
			<tr>
				<th><label for="recaptcha_type"><?php _e('reCAPTCHA Type', 'wp-mailinglist'); ?></label></th>
				<td>
					<label><input onclick="jQuery('#recaptcha_type_div_robot').show();" <?php echo (!empty($recaptcha_type) && $recaptcha_type == "robot") ? 'checked="checked"' : ''; ?> type="radio" name="recaptcha_type" value="robot" id="recaptcha_type_robot" /> <?php _e('Normal V2', 'wp-mailinglist'); ?></label>
