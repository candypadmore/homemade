...
$history_id = (empty($_POST['ishistory'])) ? esc_html($_GET['id']) : esc_html($_POST['ishistory']);	
$src = (!empty($history_id)) ? admin_url('admin-ajax.php?action=' . $this -> pre . 'history_iframe&id=' . $history_id) : false; 
$textsrc = (!empty($history_id)) ? admin_url('admin-ajax.php?action=' . $this -> pre . 'history_iframe&id=' . $history_id . '&text=1') : false; 
...
<p>
	...
	<a href="" id="previewrunnerbutton" onclick="newsletter_autosave(); return false;" class="button button-primary"><i class="fa fa-eye"></i> <?php _e('Update Preview', 'wp-mailinglist'); ?></a>
