            ...
            <?php if ($mailinglists = $Mailinglist -> select(true)) : ?>
                ...
                <div class="scroll-list">
                    <?php foreach ($mailinglists as $list_id => $list_title) : ?>
                        <div><label class="selectit"><input onclick="update_subscribers();" <?php echo (!empty($_POST['mailinglists']) && is_array($_POST['mailinglists']) && in_array($list_id, $_POST['mailinglists'])) ? 'checked="checked"' : ''; ?> type="checkbox" name="mailinglists[]" id="checklist<?php echo $list_id; ?>" value="<?php echo $list_id; ?>" /> <?php echo $list_title; ?> (<?php echo $SubscribersList -> count(array('list_id' => $list_id, 'active' => "Y")); ?> <?php _e('active', 'wp-mailinglist'); ?>)</label></div>
                    <?php endforeach; ?>
                </div>
            <?php else : ?>
	        ...
	        <div class="misc-pub-section">
	        	...
	        	<div id="daterange_div" style="display:<?php echo (!empty($_POST['daterange']) && $_POST['daterange'] == "Y") ? 'block' : 'none'; ?>;">
	        		...
	        		<p>
	        			...
	        			<input placeholder="<?php echo esc_attr(stripslashes($Html -> gen_date("Y-m-d"))); ?>" onblur="update_subscribers();" onkeyup="update_subscribers();" type="text" name="daterangeto" value="<?php echo esc_attr(stripslashes($_POST['daterangeto'])); ?>" id="daterangeto" class="widefat" style="width:120px;" />
	        		</p>
	        	</div>
	        	
	        	<script type="text/javascript">
	        	jQuery(document).ready(function() {
		        	jQuery('#daterangefrom').datepicker({showButtonPanel:true, numberOfMonths:1, changeMonth:true, changeYear:true, defaultDate:"<?php echo esc_html($_POST['daterangefrom']); ?>", dateFormat:"yy-mm-dd"});
		        	jQuery('#daterangeto').datepicker({showButtonPanel:true, numberOfMonths:1, changeMonth:true, changeYear:true, defaultDate:"<?php echo esc_html($_POST['daterangeto']); ?>", dateFormat:"yy-mm-dd"});
	        	});
	        	</script>
	        </div>
	    <?php endif; ?>
		        		...
		        		<select onchange="update_subscribers();" name="selectedcountries[]" multiple="multiple" style="width:100%;">
			        		<?php if ($countries = $this -> Country() -> select_code()) : ?>
			        			<?php foreach ($countries as $country_code => $country_name) : ?>
			        				<option <?php echo (!empty($_POST['selectedcountries']) && in_array($country_code, $_POST['selectedcountries'])) ? 'selected="selected"' : ''; ?> value="<?php echo $country_code; ?>"><?php echo $country_name; ?></option>
			        			<?php endforeach; ?>
			        		<?php endif; ?>
		        		</select>
		        	</div>
		                            ...
		                            <p>
		                                ...
		                                switch ($field -> type) {
		                                	...
		                                	case 'hidden'			:
		                                		...
		                                		$value = (empty($_POST['fields'][$field -> slug])) ? false : esc_attr(stripslashes($_POST['fields'][$field -> slug]));
...
function update_subscribers() {
	...
	srequest = jQuery.ajax({
		url: newsletters_ajaxurl + 'action=subscribercount',
		data: formvalues,
		type: "POST",
		cache: false
	...
	srequest = jQuery.post(ajaxurl, data, function(response) {
		if (response == "0") {
			...
			jQuery('#savedraftbutton, #savedraftbutton2').prop('disabled', true);
			jQuery('#subscriberscount').html('<?php echo addslashes(__('No subscribers available.', 'wp-mailinglist')); ?>');
		} else {
			delete data.action;
