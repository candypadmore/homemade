			...
			if (!empty($data)) {
				...
				switch ($method) {
					...
					case 'settings'					:
						...
						if (empty($scroll)) { $this -> data -> scroll = 0; }
		...
		function customcss($form = null) {
			...
			if (!empty($form)) {
				...
				try {
					...
					$css .= stripslashes($form -> styling_customcss);
					$css .= '}';
					
					$compiled = $scss -> compile($css);
					
					return apply_filters('newsletters_subscribeform_customcss', stripslashes($compiled), $form);
				} catch (Exception $e) {
					$error = $e -> getMessage();
					$this -> log_error($error);
					
					return stripslashes($form -> styling_customcss);
				}
			}
