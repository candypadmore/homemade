			...
			if (!empty($data)) {			
				if ($validate == true) {
					if (!empty($id)) {
						...
						if (empty($title)) { $this -> errors['title'] = __('Please fill in a title', 'wp-mailinglist'); }
						elseif ($oldfield -> slug != "email") {
							if ($Html -> sanitize($title, '_') == "email") { $this -> errors['title'] = __('You cannot create an email custom field.', 'wp-mailinglist'); }
						}
					} else {
						if (empty($title)) { $this -> errors['title'] = __('Please fill in a title', 'wp-mailinglist'); }
						elseif ($Html -> sanitize($title, '_') == "email") { $this -> errors['title'] = __('You cannot create an email custom field.', 'wp-mailinglist'); }
					}
					
					include $this -> plugin_base() . DS . 'includes' . DS . 'variables.php';
					
					if (empty($slug)) { $this -> errors['slug'] = __('Please fill in a slug/nicename for this custom field.', 'wp-mailinglist'); }
					elseif (empty($id) && empty($oldfield) && $this -> slug_exists($slug)) { $this -> errors['slug'] = __('A custom field with this slug already exists, please choose a different one.', 'wp-mailinglist'); }
					elseif (in_array($slug, $wordpress_reserved_terms)) { $this -> errors['slug'] = sprintf(__('"%s" is a reserved term, please choose something else', 'wp-mailinglist'), $slug); }
					
					if (empty($required)) { $this -> errors['required'] = __('Please choose a required status', 'wp-mailinglist'); }
					else {
						if ($required == "Y") {
							if (empty($errormessage)) { $this -> errors['errormessage'] = __('Please fill in an error message', 'wp-mailinglist'); }
						}
					}
					
					if (empty($display)) { $this -> errors['display'] = __('Please choose the display for this field.', 'wp-mailinglist'); }
					
					if (empty($type)) { $this -> errors['type'] = __('Please choose a field type', 'wp-mailinglist'); }
					else {				
