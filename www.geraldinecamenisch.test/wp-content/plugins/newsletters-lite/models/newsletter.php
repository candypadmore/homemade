	...
	class newsletters_lite extends wpMailPlugin {
		...
		function lite_admin_bar_menu($wp_admin_bar = null) {
			...
			$args = array(
				...
				'meta'		=>	array('class' => 'newsletters-lite-submitserial', 'onclick' => "jQuery.colorbox({href:newsletters_ajaxurl + \"action=" . $this -> pre . "serialkey\"}); return false;"),
			);
		...
		function lite_mailinglist_validation($errors = null, $data = null) {
			...
			if (!empty($newsletters_lite_listlimit) && $newsletters_lite_listlimit > 0) {
				...
				if (empty($data -> id) && $list_count >= $newsletters_lite_listlimit) {
					$error = sprintf(__('Mailing list limit of %s has been reached, you can %s for unlimited.', 'wp-mailinglist'), $newsletters_lite_listlimit, '<a href="' . admin_url('admin.php?page=' . $this -> sections -> lite_upgrade) . '">Upgrade to PRO</a>');
					$errors['limit'] = $error;
