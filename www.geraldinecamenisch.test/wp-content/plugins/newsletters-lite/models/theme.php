	...
	class wpmlTheme extends wpMailPlugin {
		...
		function validate($data = array()) {
			...
			if (!empty($data)) {
				...
				if (empty($type)) { $this -> errors['type'] = __('Please choose a submission type', 'wp-mailinglist'); }
				else {				
					switch ($type) {
						case 'upload'		:
							if ($_FILES['upload']['error'] > 0) {
								...
								if (empty($_FILES['upload']['name'])) { $this -> errors['upload'] = __('Please choose an HTML file for uploading', 'wp-mailinglist'); }
								elseif (!is_uploaded_file($_FILES['upload']['tmp_name'])) { $this -> errors['upload'] = __('HTML file could not be uploaded', 'wp-mailinglist'); }
								elseif ($_FILES['upload']['type'] != "text/html") { $this -> errors['upload'] = __('This is not a valid HTML file. Ensure that it has a .html extension', 'wp-mailinglist'); }
								else {
