	...
	class wpmlSubscriber extends wpMailPlugin {
					...
					if (!empty($data -> recursive) && $data -> recursive == true) {									
						...
						if (!empty($val)) {
							if (!in_array($key, $this -> table_fields)) {
								...
								//$_REQUEST[$key] = esc_html($_POST[$key]) = $val;
							}
						}
					}
				}
		...
		function mailinglists($subscriber_id = null, $includeonly = null, $exclude = null, $active = "Y") {
			...
			if (!empty($subscriber_id)) {
				...
				if (!empty($active)) { $query .= " AND `active` = '" . esc_sql($active) . "'"; }
		...
		function get_segmented_query($fields = null, $scopeall = null, $condquery) {
		...
		function search($data = array()) {
			...
			if (!empty($data)) {
				if (empty($data['searchterm'])) { $this -> errors[] = __('Please fill in a searchterm', 'wp-mailinglist'); }
				if (empty($data['searchtype'])) { $this -> errors[] = __('Please select a search type', 'wp-mailinglist'); }
				...
				if (empty($this -> errors)) {
					if ($data['searchtype'] == "listtitle") {
						...
						if (!empty($lists)) {
							...
							$this -> errors[] = __('No mailing lists matched your title', 'wp-mailinglist');
							return false;
						}
					} else {
				...
				if ($validate == true) {				
					...
					if ($captcha_type = $this -> use_captcha($cap)) {						
						if ($captcha_type == "rsc") {						
							...
							if (empty($data['captcha_code'])) { $this -> errors['captcha_code'] = __('Please fill in the code in the image.', 'wp-mailinglist'); }
		...
		function save($data = array(), $validate = true, $return_query = false, $skipsubscriberupdate = false, $emptyfields = false) {
			...
			$ipaddress = (is_admin() && !defined('DOING_AJAX')) ? false : $_SERVER['REMOTE_ADDR'];
			...
			if ($validate == true) {			
				...
				} elseif (!$this -> email_validate($email)) { $this -> errors['email'] = __('Please fill in a valid email address', 'wp-mailinglist'); }
				...
				if (empty($registered)) { $registered = "N"; /*$this -> errors['registered'] = __('Please select a registered status', 'wp-mailinglist');*/ }
				elseif ($registered == "Y") {
					if (!$userid = $this -> check_registration($data['email'])) {
						if (empty($username)) { $this -> errors['username'] = __('Please fill in a username', 'wp-mailinglist'); }
						else {								
				...
				if (empty($active)) { $this -> errors['active'] = __('Please select an active status', 'wp-mailinglist'); }
			} else {
				if (empty($email)) { $this -> errors['email'] = __($emailfield -> errormessage); }
			}
					...
					if (!empty($fields)) {				
						foreach ($fields as $field) {
							if ((empty($usedfields)) || (!empty($usedfields) && !in_array($field -> slug, $usedfields))) {						
								if (!empty($data[$field -> slug]) || $data[$field -> slug] == "0" || $field -> type == "file") {																								
									if ($field -> type == "file") {																					
				...
				if (empty($return_query) || $return_query == false) {			
					...
					if ($result !== false && $result >= 0) {											
						...
						if ($this -> get_option('subscriberegister') == "Y") {
							...
							$subscriberquery = "UPDATE `" . $wpdb -> prefix . $this -> table . "` SET `registered` = 'Y', `user_id` = '" . $user_id . "' WHERE `id` = '" . esc_sql($subscriber_id) . "'";
