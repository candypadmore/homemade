...
if (!class_exists('wpmlFieldsList')) {
	class wpmlFieldsList extends wpMailPlugin {
		...
		function count_by_list($list_id = null) {
			...
			if (!empty($list_id)) {		
				if ($list = $Mailinglist -> get($list_id)) {
					$query = "SELECT COUNT(*) FROM `" . $wpdb -> prefix . "" . $this -> table_name . "` WHERE `list_id` = '" . esc_sql($list_id) . "'";
		...
		function save($data = array(), $validate = true) {
			...
			if (!empty($data)) {
				if ($validate == true) {
					if (empty($data['field_id'])) { $this -> errors[] = __('No field was specified', 'wp-mailinglist'); }
					if (empty($data['list_id']) && $data['list_id'] != "0") { $this -> errors[] = __('No list was specified', 'wp-mailinglist'); }
				} else {
					$this -> errors = false;
				}
			
				if (empty($this -> errors)) {
					$query = "INSERT INTO `" . $wpdb -> prefix . "" . $this -> table_name . "` (`field_id`, `list_id`, `special`) VALUES ('" . esc_sql($data['field_id']) . "', '" . esc_sql($data['list_id']) . "', '" . esc_sql($data['special']) . "');";
