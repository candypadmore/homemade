...
if (!class_exists('wpmlHistory')) {
	class wpmlHistory extends wpmlDbHelper {
		...
		function queue_recurring( ) {
			...
			echo sprintf(__('%s recurring emails queued', 'wp-mailinglist'), $recurring_queued) . '<br/>';
		...
		function queue_scheduled() {
			...
			echo sprintf(__('%s scheduled emails queued', 'wp-mailinglist'), $scheduled_queued) . '<br/>';
			return true;
		}
		...
		function save($data = array(), $validate = true, $insertpost = true) {
			...
			if ($validate == true) {
				if (empty($subject)) { $this -> errors['subject'] = __('No subject specified', 'wp-mailinglist'); }
