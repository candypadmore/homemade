		...
		function save($data = array(), $validate = true) {
			...
			if (!empty($data)) {			
				if ($validate == true) {				
					...
					if (empty($privatelist)) { $this -> errors['privatelist'] = __('Please select private status', 'wp-mailinglist'); }
					...
					if (empty($paid)) {
						...
						if ($paid == "Y") {
							if ($this -> get_option('paymentmethod') == "2co") {
								if (empty($tcoproduct)) { $this -> errors['tcoproduct'] = __('Please fill in a valid 2Checkout product ID', 'wp-mailinglist'); }
							}
							
							if (empty($interval)) { $this -> errors['interval'] = __('Please select a subscription interval', 'wp-mailinglist'); }
							if (empty($price)) { $this -> errors['price'] = __('Please fill in a subscription price', 'wp-mailinglist'); }
						}
					}
				}
