...
if (!class_exists('WP_Queue_Process_2')) {
	class WP_Queue_Process_2 extends WP_Background_Process {
	    ...
	    function get_queued_count($key = null) {
		    ...
		    if ($queue_count = get_transient('newsletters_queue_count')) {
			    return $queue_count;
		    }
		...
		public function handle_cron_healthcheck() {					
			if ( $this->is_process_running() ) {
				...
				echo __('The process is already running');
				exit;
			}
			
			$queue_status = get_option('wpmlqueue_status');
			if (!empty($queue_status) && $queue_status == "pause") {
				echo __('The queue is currently paused');
				exit();
			}

			if ( $this->is_queue_empty() ) {
				echo __('The queue is currently empty');
		...
		protected function handle() {		
			...
			echo sprintf(__('%s emails have been sent out'), $successful);
			return true;
		}
