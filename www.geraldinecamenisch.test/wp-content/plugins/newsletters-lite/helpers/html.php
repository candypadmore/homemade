...
if (!class_exists('wpmlHtmlHelper')) {
	class wpmlHtmlHelper extends wpMailPlugin {
		...
		public function detectDelimiter($csvFile = null) {
		    ...
		    $firstLine = stripslashes(fgets($handle));
		    fclose($handle); 
		...
		function file_custom_field($value = null, $limit = false, $types = false, $field = null, $removefile = false) {	
			...
			if (!empty($value)) {
				...
				$currentfile .= '<a class="btn btn-default btn-sm" href="' . esc_attr(stripslashes($value)) . '" target="_blank"><i class="fa fa-paperclip"></i> ' . __('Uploaded file', 'wp-mailinglist') . '</a>';			
